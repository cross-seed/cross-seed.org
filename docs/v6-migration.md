...
Since the initial pre-release (`6.0.0-0`) we've worked with our users and friends to iron out implementations in the most reasonable and usable way possible. As we proceed through the pre-release/testing phase for v6, there may be alterations to things further. They will all be documented here prior to releases.

The majority of the trade-offs we've had to make have been for enhancements and meaningful features. While this update will require some users to rethink, or reconfigure, certain aspects of their setup or workflow to accomodate this; these changes were unavoidable unfortunately.

The enhancements we've added and changes we've made are outlined in this migration guide.

:::tip QBittorrent
We previously advised qBittorrent users to hold off on the initial pre-release (`6.0.0-0`), we feel we now have `cross-seed` in a state which is usable for those users. There are [some changes](#qbittorrent) which should be noted prior to updating to help test, however if you make backups and perform any necessary adjustments to your workflow, it should be in a usable state now.
:::
