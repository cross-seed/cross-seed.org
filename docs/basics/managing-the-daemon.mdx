...
If you are using Windows 11, you can use the built-in Task Scheduler to run `cross-seed daemon` at start up. (This should work with any Windows version after Vista, but has only been tested on 11).
...
6. Configure the launch command:
    ...
    a) In the "Program/script" box, paste the path to your cross-seed install. For example, if your node installation is managed by Windows NVM, and you installed cross-seed with `npm install -g cross-seed`, this will be found at "C:\Program Files\nodejs\cross-seed.cmd'. If unsure, run `where cross-seed` in a command line terminal.
...
9. Finally, test that this works by restarting your PC and checking the cross-seed logs for activity
