...
You can also try `ExecStart=npx cross-seed daemon` if the one above doesn't
work.
...
6. Configure the launch command:
    ...
    used to launch it by using the command `where cross-seed` from a command
    prompt/terminal. Simply paste the path in the "Program/script" box.
