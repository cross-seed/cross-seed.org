...
Torrents that were successfully injected or already existed in client will have their .torrent file removed from the directory being processed if they are complete. Torrent files older than 1 week (by last modified time) will also be removed regardless of injection status from the directory.
